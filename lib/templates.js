...
const beforeRender = (reporter) => async (req, res) => {
  if (!req.template._id && !req.template.shortid && !req.template.name) {
    ...
    reporter.logger.info('Rendering anonymous template { recipe:' +
  ...
  reporter.logger.info(`Rendering template {name: ${req.template.name}, recipe: ${req.template.recipe}, engine: ${req.template.engine}, preview: ${(req.options.preview || false)}}`, req)
...
module.exports = function (reporter, definition) {
  ...
  reporter.initializeListeners.add('templates', function () {
    ...
    col.beforeInsertListeners.add('templates', (doc) => {
      if (!doc.engine) {
        const e = new Error('Template must contain engine')
        e.weak = true
        throw e
      }
      if (!doc.recipe) {
        const e = new Error('Template must contain recipe')
        e.weak = true
        throw e
      }
