...
const beforeRender = (reporter) => async (req, res) => {
  ...
  async function findTemplate () {
    async function findQuery (useFolders = true) {
      if (req.template._id) {
      ...
      if (req.template.name) {
        ...
        return {
          name: useFolders ? req.template.name.split('/').pop() : req.template.name,
          ...(useFolders && folder && { folder: { shortid: folder.shortid } })
        }
      }
    }

    const query = await findQuery(true)

    if (!query) {
      return req.template
    }

    const template = await reporter.documentStore.collection('templates').findOne(query, req)
    if (!template) {
      return reporter.documentStore.collection('templates').findOne(await findQuery(false), req)
    }
    return template
  }
