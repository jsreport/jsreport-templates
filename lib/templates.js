...
const beforeRender = (reporter) => async (req, res) => {
  ...
  if (
    req.context.renderHierarchy.length > 0 &&
    template._id != null &&
    req.context.renderHierarchy.some((tid) => tid != null && tid === template._id)
  ) {
    throw reporter.createError('Render cycle detected. Template (_id: ' + template._id + ') was rendered previously in this render request. Please verify that reporter.render is not causing cycle', {
      weak: true,
      statusCode: 403
    })
  }
